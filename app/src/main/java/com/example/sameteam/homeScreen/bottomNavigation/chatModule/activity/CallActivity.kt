...
import com.example.sameteam.helper.Constants.EXTRA_INCOMING_GROUP_NAME
import com.example.sameteam.helper.Constants.EXTRA_IS_INCOMING_CALL
import com.example.sameteam.helper.Constants.EXTRA_LOGIN_RESULT
import com.example.sameteam.helper.Constants.EXTRA_LOGIN_RESULT_CODE
import com.example.sameteam.helper.Constants.PERMISSIONS
import com.example.sameteam.helper.PermissionsActivity
...
import com.example.sameteam.quickBlox.chat.ChatHelper.loadUsersByIds
import com.example.sameteam.quickBlox.db.QbUsersDbManager
import com.example.sameteam.quickBlox.fragments.*
...
class CallActivity : BaseActivity2(), IncomeCallFragmentCallbackListener, QBRTCSessionStateCallback<QBRTCSession>,
    ...
    companion object {
        fun start(context: Context, isIncomingCall: Boolean, groupName: String) {
            val intent = Intent(context, CallActivity::class.java)
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            intent.putExtra(EXTRA_IS_INCOMING_CALL, isIncomingCall)
            intent.putExtra(EXTRA_INCOMING_GROUP_NAME, groupName)
            SharedPrefs.saveGroupName(context, groupName)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d(TAG, "onCreate: Call Activity")
...
//            }
            ...
            if (intent != null && intent.extras != null) {
                ...
                groupName = intent?.extras?.getString(EXTRA_INCOMING_GROUP_NAME) ?: ""
            } else {
...
//    }
    ...
    private fun checkPermission() {
        val cam = SharedPrefsHelper[PERMISSIONS[0], true]
        val mic = SharedPrefsHelper[PERMISSIONS[1], true]
...
//        startActivityForResult(intent, REQUEST_PERMISSION_SETTING)
    ...
    private fun startLoadAbsentUsers() {
        ...
        if (idsNotLoadedUsers.isNotEmpty()) {
            loadUsersByIds(idsNotLoadedUsers, object : QBEntityCallback<ArrayList<QBUser>> {
    ...
    private fun initIncomingCallTask() {
        showIncomingCallWindowTaskHandler = Looper.myLooper()?.let { Handler(it) }!!
...
//        ChatActivity.start(this)
    ...
    private fun addConversationFragment(isIncomingCall: Boolean) {
        ...
        val conversationFragment = BaseConversationFragment.newInstance(baseConversationFragment, isIncomingCall, groupName)
        addFragment(
    ...
    override fun onReceiveHangUpFromUser(session: QBRTCSession?, userId: Int?, map: MutableMap<String, String>?) {
        if (callService.isCurrentSession(session)) {
            if (userId == session?.callerID) {
                ...
                val participantName = if (participant != null) participant.fullName else ""
                shortToast(participantName + " " + getString(R.string.text_status_hang_up) + " conversation")
            }
    ...
    override fun onChangeReconnectionState(
        p0: QBRTCSession?,
        p1: Int?,
        p2: QBRTCTypes.QBRTCReconnectionState?
    ) {
    ...
    private inner class CallServiceConnection : ServiceConnection {
        ...
        private fun login() {
            ...
            val pendingIntent: PendingIntent = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                createPendingResult(EXTRA_LOGIN_RESULT_CODE, tempIntent, PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT)
            } else{
                createPendingResult(EXTRA_LOGIN_RESULT_CODE, tempIntent, 0)
            }
            if (qbUser != null) {
