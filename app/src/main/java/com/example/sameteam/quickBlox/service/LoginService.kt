...
import com.example.sameteam.helper.Constants.EXTRA_LOGIN_ERROR_MESSAGE
...
import com.example.sameteam.quickBlox.util.configRTCTimers
...
private const val COMMAND_START = 3
...
//            intent.putExtra(EXTRA_PENDING_INTENT, pendingIntent)
            intent.putExtra(EXTRA_COMMAND_TO_SERVICE, COMMAND_START )

            context.startService(intent)
        }

        fun startBackground(context: Context, qbUser: QBUser, pendingIntent: PendingIntent? = null) {
            val intent = Intent(context, LoginService::class.java)
            intent.putExtra(EXTRA_COMMAND_TO_SERVICE, COMMAND_LOGIN)
            intent.putExtra(EXTRA_QB_USER, qbUser)
            intent.putExtra(EXTRA_PENDING_INTENT, pendingIntent)
    ...
    private fun startSuitableActions() {
        if(currentCommand == COMMAND_START){
//            initPingListener()
            initQBRTCClient()
        } else if (currentCommand == COMMAND_LOGIN) {
            startLoginToChat()
