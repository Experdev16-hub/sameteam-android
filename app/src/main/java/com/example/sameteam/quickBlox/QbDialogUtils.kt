...
fun createDialog(users: MutableList<QBUser>, chatName: String, isPrivate: Boolean?): QBChatDialog {
//    if (isPrivateChat(users)) {
//        users.remove(ChatHelper.getCurrentUser())
//    } removed this to create dialog with 2 participants
...
private fun logUsersByIds(usersIds: List<Int>) {
    for (id in usersIds) {
        val user = QbUsersDbManager.getUserById(id)
        Log.i(TAG, user?.id.toString() + " " + user?.fullName)
    }
}
...
fun getDialogName(dialog: QBChatDialog): String {
    ...
    Log.d(TAG, "getDialogName: ${dialog.occupants.get(0)}")
    return result
}
