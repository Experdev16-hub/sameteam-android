package com.example.sameteam.quickBlox.service
...
import com.example.sameteam.quickBlox.base.MIC_ENABLED
import com.example.sameteam.quickBlox.db.QbUsersDbManager
import com.example.sameteam.quickBlox.fragments.CAMERA_ENABLED
import com.example.sameteam.quickBlox.fragments.IS_CURRENT_CAMERA_FRONT
import com.example.sameteam.quickBlox.fragments.SPEAKER_ENABLED
...
//        )
        ...
        builder.color = ContextCompat.getColor(MyApplication.getInstance(), R.color.colorPrimary )
        builder.setSmallIcon(R.drawable.ic_app_icon)
        val bitmapIcon = BitmapFactory.decodeResource(resources, R.drawable.ic_app_icon)
    ...
    private fun createNotificationChannel(channelId: String, channelName: String): String {
        val channel =
            NotificationChannel(channelId, channelName, NotificationManager.IMPORTANCE_LOW).apply { lockscreenVisibility = Notification.VISIBILITY_PUBLIC }
...
//            shortToast("Headset " + if (plugged) "plugged" else "unplugged")
            Log.d(TAG, "Headset" + if (plugged) "plugged" else "unplugged")
        }

        appRTCAudioManager.setBluetoothAudioDeviceStateListener { connected ->
            Log.d(TAG,"Bluetooth " + if (connected) "connected" else "disconnected")
        }
...
//    }
    ...
    private inner class ConnectionListenerImpl : AbstractConnectionListener() {
        override fun connectionClosedOnError(e: Exception?) {
            ...
            val reconnectHangUpTimeMillis = getPreferenceInt(
    ...
    private inner class SessionEventsListener : QBRTCClientSessionCallbacks {
        ...
        override fun onReceiveHangUpFromUser(
            ...
            val participantName =
                if (participant != null) participant.fullName else ""
...
//            shortToast("$participantName has disconnected the call")
