...
import com.example.sameteam.quickBlox.chat.CURRENT_PAGE_BUNDLE_PARAM
import com.example.sameteam.quickBlox.chat.ChatHelper
import com.example.sameteam.quickBlox.chat.TOTAL_PAGES_BUNDLE_PARAM
import com.example.sameteam.quickBlox.chat.USERS_PER_PAGE
...
import com.example.sameteam.quickBlox.callback.QbEntityCallbackImpl
import com.example.sameteam.quickBlox.db.QbUsersDbManager
import com.example.sameteam.quickBlox.getOccupantsIdsStringFromList
import com.example.sameteam.quickBlox.getOccupantsNamesStringFromList
...
const val OCCUPANTS_REMOVED = "4"
const val PROPERTY_NEW_OCCUPANTS_IDS = "new_occupants_ids"
const val PROPERTY_OLD_OCCUPANTS_IDS = "old_occupants_ids"
...
class DialogsManager {
    ...
    private fun isMessageRemovedUser(message: QBChatMessage): Boolean {
        return OCCUPANTS_REMOVED == message.getProperty(PROPERTY_NOTIFICATION_TYPE)
    }
    ...
    private fun buildMessageCreatedGroupDialog(dialog: QBChatDialog): QBChatMessage {
        ...
        qbChatMessage.body = MyApplication.getInstance().getString(R.string.new_chat_created, getCurrentUserName(), dialog.name)
    ...
    private fun buildMessageAddedUsers(dialog: QBChatDialog, userIds: String, usersNames: String): QBChatMessage {
        ...
        qbChatMessage.body = MyApplication.getInstance().getString(R.string.occupant_added, getCurrentUserName(), usersNames)
        qbChatMessage.setSaveToHistory(true)
        qbChatMessage.isMarkable = true
        return qbChatMessage
    }

    private fun buildMessageRemovedUsers(dialog: QBChatDialog, userIds: String, usersNames: String): QBChatMessage {
        val qbChatMessage = QBChatMessage()
        qbChatMessage.dialogId = dialog.dialogId
        qbChatMessage.setProperty(PROPERTY_NOTIFICATION_TYPE, OCCUPANTS_REMOVED)
        qbChatMessage.setProperty(PROPERTY_OLD_OCCUPANTS_IDS, userIds)
        qbChatMessage.body = MyApplication.getInstance().getString(R.string.occupant_removed, getCurrentUserName(), usersNames)
        qbChatMessage.setSaveToHistory(true)
        qbChatMessage.isMarkable = true
        return qbChatMessage
    }
    ...
    private fun buildMessageLeftUser(dialog: QBChatDialog): QBChatMessage {
        ...
        qbChatMessage.body = MyApplication.getInstance().getString(R.string.occupant_left, getCurrentUserName())
        qbChatMessage.setSaveToHistory(true)
        qbChatMessage.isMarkable = true
        return qbChatMessage
    }
    ...
    fun sendMessageAddedUsers(dialog: QBChatDialog, newUsersIds: List<Int>) {
        ...
        loadNewUsersByIDs(newUsersIds, requestBuilder, ArrayList(), object : QBEntityCallback<ArrayList<QBUser>> {
            ...
            override fun onError(e: QBResponseException?) {
                ...
                Toast.makeText(MyApplication.getInstance(),"Failed to load users to send message",Toast.LENGTH_SHORT).show()
            }
        })
    }

    fun sendMessageRemovedUsers(dialog: QBChatDialog, oldUsersIds: List<Int>) {
        val requestBuilder = QBPagedRequestBuilder(USERS_PER_PAGE, 1)
        loadNewUsersByIDs(oldUsersIds, requestBuilder, ArrayList(), object : QBEntityCallback<ArrayList<QBUser>> {
            override fun onSuccess(qbUsers: ArrayList<QBUser>?, b: Bundle?) {
                val usersIds = getOccupantsIdsStringFromList(oldUsersIds)
                if (oldUsersIds.isNotEmpty() && qbUsers != null) {
                    val usersNames = getOccupantsNamesStringFromList(qbUsers)
                    val messageUsersRemoved = buildMessageRemovedUsers(dialog, usersIds, usersNames)

                    sendMessageHandleJoining(dialog, messageUsersRemoved)
                }
            }
    ...
    private fun sendMessageHandleJoining(dialog: QBChatDialog, qbChatMessage: QBChatMessage) {
        if (dialog.isJoined) {
            ...
            ChatHelper.join(dialog, object : QBEntityCallback<Void> {
                ...
                override fun onError(e: QBResponseException?) {
                    Toast.makeText(MyApplication.getInstance(),e?.message,Toast.LENGTH_SHORT).show()
    ...
    fun sendSystemMessageAddedUser(systemMessagesManager: QBSystemMessagesManager, dialog: QBChatDialog, newUsersIds: List<Int>) {
        ...
        loadNewUsersForSystemMsgsByIDs(newUsersIds, requestBuilder, ArrayList(), object : QBEntityCallback<ArrayList<QBUser>> {
            ...
            override fun onError(e: QBResponseException?) {
                ...
                Toast.makeText(MyApplication.getInstance(),"Failed to load users to send system message",Toast.LENGTH_SHORT).show()

            }
        })
    }

    fun sendSystemMessageRemovedUser(systemMessagesManager: QBSystemMessagesManager, dialog: QBChatDialog, oldUsersIds: List<Int>) {
        val requestBuilder = QBPagedRequestBuilder(USERS_PER_PAGE, 1)
        loadNewUsersForSystemMsgsByIDs(oldUsersIds, requestBuilder, ArrayList(), object : QBEntityCallback<ArrayList<QBUser>> {
            override fun onSuccess(qbUsers: ArrayList<QBUser>?, bundle: Bundle?) {
                val usersIds = getOccupantsIdsStringFromList(oldUsersIds)
                if (oldUsersIds.isNotEmpty() && qbUsers != null) {
                    val usersNames = getOccupantsNamesStringFromList(qbUsers)
                    ...
                    val messageUsersRemoved = buildMessageRemovedUsers(dialog, usersIds, usersNames)
                    prepareSystemMessage(systemMessagesManager, messageUsersRemoved, dialog.occupants)
                }
            }
