package com.example.sameteam.widget.simplecropview;
...
import com.example.sameteam.widget.simplecropview.animation.SimpleValueAnimator;
import com.example.sameteam.widget.simplecropview.animation.SimpleValueAnimatorListener;
import com.example.sameteam.widget.simplecropview.animation.ValueAnimatorV14;
import com.example.sameteam.widget.simplecropview.callback.Callback;
...
import com.example.sameteam.widget.simplecropview.util.Logger;
import com.example.sameteam.widget.simplecropview.util.ThreadPoolQueue;
import com.example.sameteam.widget.simplecropview.util.Utils;
...
//      e.printStackTrace();
//    }
    mExecutor = new ThreadPoolExecutor(0, n, 1, TimeUnit.MINUTES, new ThreadPoolQueue(n));
  ...
  private float constrain(float val) {
    if (val < (float) 0.01 || val > (float) 1.0) return CropImageView.DEFAULT_INITIAL_FRAME_SCALE;
    return val;
  }
  ...
  public Completable loadAsCompletable(final Uri sourceUri, final boolean useThumbnail,
    ...
    }).doOnSubscribe(disposable -> mIsLoading.set(true)).doFinally(() -> mIsLoading.set(false));
  }
  ...
  public Single<Bitmap> cropAsSingle(final Uri sourceUri) {
    return Single.fromCallable(() -> {
      if (sourceUri != null) mSourceUri = sourceUri;
      return cropImage();
    }).doOnSubscribe(disposable -> mIsCropping.set(true)).doFinally(() -> mIsCropping.set(false));
  }
  ...
  public Single<Uri> saveAsSingle(final Bitmap bitmap, final Uri saveUri) {
    return Single.fromCallable(() -> saveImage(bitmap, saveUri)).doOnSubscribe(disposable -> mIsSaving.set(true)).doFinally(() -> mIsSaving.set(false));
  }
  ...
  public void setInitialFrameScale(float initialScale) {
    mInitialFrameScale = constrain(initialScale);
  }
